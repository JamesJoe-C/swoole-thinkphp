...
class Dispatcher
{
    ...
    public static function dispatch()
    {
        ...
        if (empty($_SERVER['PATH_INFO'])) {
            ...
            // URL后缀
            define('__EXT__', strtolower(pathinfo($_SERVER['PATH_INFO'], PATHINFO_DIRNAME)));
            $_SERVER['PATH_INFO'] = trim($_SERVER['PATH_INFO'], '/');
            if (!defined('BIND_MODULE') && (!C('URL_ROUTER_ON') || !Route::check())) {
                if ($_SERVER['PATH_INFO'] && C('MULTI_MODULE')) { // 获取模块名
        ...
        //保证$_REQUEST正常取值
        $_REQUEST = array_merge($_POST, $_GET); // -- 加了$_COOKIE.  保证哦..  保证个毛坑死人
    }
